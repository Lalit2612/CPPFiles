...
Say the bag that used to contain X candies (before eating), now contains [X/2] candies! ,where [x] is the greatest integer less than x (Greatest Integer Function).
Amazed by the magical spell, Monk can now have a lot more candies! But he has to return home in K minutes. In a single minute,Monk can consume all the candies in a
single bag, regardless of the number of candies in it.
...
int main(){
    ...
    while(q--){
        int n, k;
        cin>>n>>k;
        multiset <long long int> candy;
        for(int i = 0; i < n; i++){
            long long int cand;
            cin>>cand;
            candy.insert(cand);
        }
        long long int cand_count = 0;
        for(int i = 0; i < k; i++){
            auto it = --candy.end();
            long long int c = *it;
            cand_count += c;
